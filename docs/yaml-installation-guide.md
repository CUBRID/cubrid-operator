...
이 가이드는 YAML 매니페스트를 사용하여 CUBRID Operator를 설치하는 방법을 상세히 설명합니다.
...
1. YAML 매니페스트로 설치 시 기본적으로 내부 cert-manager가 사용됩니다.
...
2. 파드 로그:
```bash
kubectl logs -n cubrid deploy/cubrid-operator-dep
